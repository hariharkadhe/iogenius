      ...
      <main style={{ flex: 1 }}>
        ...
        <section style={{ padding: '6rem 2rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(59, 130, 246, 0.1)', color: 'var(--primary)', padding: '0.5rem 1rem', borderRadius: '50px', border: '1px solid rgba(59,130,246,0.2)', marginBottom: '2rem', fontWeight: 600 }}>
            ...
            <h1 style={{ fontSize: '4.5rem', fontWeight: 800, lineHeight: 1.1, marginBottom: '1.5rem', letterSpacing: '-1px' }}>
              Turn your <span className="text-gradient">ideas</span> into <br />
        ...
        <section style={{ padding: '4rem 2rem', maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            <motion.div whileHover={{ y: -5 }} className="glass-card" style={{ padding: '2rem' }}>
              <div style={{ background: 'rgba(59,130,246,0.1)', color: 'var(--primary)', width: '60px', height: '60px', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
                <BrainCircuit size={32} />
              </div>
              <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem' }}>AI Requirement Engine</h3>
              <p style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}>Simply type what you want to build. Our AI reads your prompt and intelligently extracts all hardware, sensor, and logical requirements instantly.</p>
            </motion.div>

            <motion.div whileHover={{ y: -5 }} className="glass-card" style={{ padding: '2rem' }}>
              <div style={{ background: 'rgba(139,92,246,0.1)', color: 'var(--accent)', width: '60px', height: '60px', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
                <Cpu size={32} />
              </div>
              <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem' }}>Smart Circuit Mapping</h3>
              <p style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}>No more guessing which pin goes where. Get an auto-generated, visually clean wiring map matching your specific microcontroller and sensors.</p>
            </motion.div>

            <motion.div whileHover={{ y: -5 }} className="glass-card" style={{ padding: '2rem' }}>
              <div style={{ background: 'rgba(16,185,129,0.1)', color: 'var(--success)', width: '60px', height: '60px', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
                <Database size={32} />
              </div>
              <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem' }}>Deployable Code</h3>
              <p style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}>Generate production-ready C++ (for Arduino/ESP32) and Python (for Raspberry Pi) code, fully integrated with cloud data transmission.</p>
            </motion.div>
          </div>
        </section>
      </main>
